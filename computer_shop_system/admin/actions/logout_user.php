...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['user_id'])) {
    ...
    // Find active session
    $active_session = $conn->query("SELECT session_id FROM user_sessions WHERE user_id=$user_id AND status='active' LIMIT 1");

    if ($active_session && $active_session->num_rows > 0) {
        $session = $active_session->fetch_assoc()['session_id'];

        // End the active session
        $conn->query("UPDATE user_sessions SET status='ended', end_time=NOW() WHERE session_id=$session");

        // Free up the computer
        $conn->query("UPDATE computers SET status='available' WHERE computer_id=(SELECT active_pc_id FROM users WHERE user_id=$user_id)");

        // Remove user's active_pc_id
        $conn->query("UPDATE users SET active_pc_id=NULL WHERE user_id=$user_id");

        // Log the action
        $admin_id = $_SESSION['admin_id'];
        $activity = "Forced logout for user ID $user_id";
...
<head>
    <meta charset="UTF-8">
    <title>Force Logout - Computer Shop System</title>
