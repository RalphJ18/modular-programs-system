...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['user_id'])) {
    ...
    if ($user_id > 0) {
        ...
        $conn->query("DELETE FROM topup_history WHERE user_id=$user_id");
        $conn->query("DELETE FROM payments WHERE topup_id IN (SELECT topup_id FROM topups WHERE user_id=$user_id)");
        $conn->query("DELETE FROM topups WHERE user_id=$user_id");
        $conn->query("DELETE FROM balance_logs WHERE user_id=$user_id");
        ...
        $activity = "Deleted user ID $user_id and all related data.";
...
<head>
    <meta charset="UTF-8">
    <title>Delete User - Computer Shop System</title>
